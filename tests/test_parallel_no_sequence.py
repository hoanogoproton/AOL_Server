...
trinh S1->S3 (2 anh) doc lap; anh cua No khac khong con lam abort cycle
...
def assign_cycle(db, no, side="L", root="", cycle_number=1) -> dict:
    """Gan day du chu trinh 2 anh (S1 -> S3) cho tube, tra ve ket qua
    step cuoi."""
    result = None
...
def test_interleaved_nos_all_assigned_no_abort(tmp_path):
    """
    Scenario chinh: 2 No xen ke S1, S1, S3, S3 tren cung side.
    ...
    # Ca 4 anh deu co event (khong file nao bi nuot).
    for no in ("No1", "No2"):
        for step in (1, 3):
...
def test_step1_fail_only_affects_that_no(tmp_path):
    ...
    ven va van cho Step 3 binh thuong.
    """
    db = make_db(tmp_path)
    ...
    assert stream_no2["next_step"] == 3
    assert stream_no2["current_cycle_id"] == cycle_no2

    # S3 cua No2 van duoc gan dung cycle cu.
...
def test_timeout_only_aborts_stuck_stream(tmp_path):
    ...
    # No1 da den S1 (dang cho S3), No2 moi den S1 (cung dang cho S3).
...
def test_reset_stream_side_aborts_all_pending_cycles(tmp_path):
    ...
    # L: No1 cho S3, No2 cho S3. R: No1 cho S3 - khong lien quan.
    assign_step(db, "No1", 1, root=str(tmp_path))
    assign_step(db, "No2", 1, root=str(tmp_path))
